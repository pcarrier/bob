...
class LaunchService {
  ...
  static const String _apiPrefix = '/api/cli/0';
  ...
  Future<LaunchResult> launch({
    ...
    try {
      ...
      // Step 1: Create bundle node structure from project files
      _currentStep = LaunchStep(
      ...
      // Step 2: Suggest bundle to check if already present
      _currentStep = LaunchStep(
      ...
      final suggestUrl = '${target.service}$_apiPrefix/suggest';
      _logToStep('Connecting to ${target.service}', onStepUpdate);
      ...
      if (suggestResponse.missing.isNotEmpty) {
        ...
        onStepUpdate?.call(_currentStep!);
        final missingUrl = '${target.service}$_apiPrefix/missing';
      ...
      if (suggestResponse.present) {
        ...
        onStepUpdate?.call(_currentStep!);
        final finalizeUrl = '${target.service}$_apiPrefix/finalize';
      ...
      final uploadUrl = '${target.service}$_apiPrefix/bundle';
      ...
      // Step 4: Upload missing parts if any
  ...
  Future<Uint8List> _encodeRequest(
    ...
    final compressed = await Uint8List.fromList(encoded).compress();
  ...
  Future<(CborMap, int)> _makeApiRequest(
    ...
    final response = await _makeRequest(
      ...
      () => _httpClient.post(
        Uri.parse(url),
        headers: {'Content-Type': 'application/cbor+zstd'},
        body: compressedBody,
      ),
    );
    ...
    final decompressed = await response.bodyBytes.decompress();
    if (decompressed == null) {
      throw Exception('Failed to decompress response');
    }
